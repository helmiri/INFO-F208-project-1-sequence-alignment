...
class Matrix:
    def __init__(self, nrows=0, ncols=0, value=None):
        self.__matrix = np.full([nrows, ncols], value)
...
class ScoreMatrix(Matrix):
    ...
    def __init__(self, seq1, seq2, I, E, matType, is_global=True):
        ...
        super().__init__(len(seq1) + 1, len(seq2) + 1, 0)
        if matType == "S":
            ...
            for j in range(self.get_num_cols()):
                self.set_value(0, j,  np.iinfo(np.int32).min)

        elif matType == "W":
            for i in range(self.get_num_rows()):
                self.set_value(i, 0, np.iinfo(np.int32).min)
